...
void Game_Interpreter::Update(bool reset_loop_count) {
	...
	for (; loop_count < loop_limit; ++loop_count) {
		...
		// Chaos Fork: AI Rewriter — show overflow boxes between commands
		// This runs in the main loop so it doesn't interfere with command indices.
		if (Chaos::AIRewriter::IsEnabled() && Chaos::AIRewriter::HasOverflow()) {
			if (Game_Message::CanShowMessage(main_flag)) {
				auto overflow = Chaos::AIRewriter::PopOverflow();
				PendingMessage ovf_pm(Game_Message::CommandCodeInserter);
				ovf_pm.SetIsEventMessage(true);
				ovf_pm.SetFromForegroundInterpreter(main_flag);
				for (auto& line : overflow) {
					ovf_pm.PushLine(std::move(line));
				}
				Game_Message::SetPendingMessage(std::move(ovf_pm));
...
bool Game_Interpreter::CheckGameOver() {
	if (!Game_Battle::IsBattleRunning() && !Main_Data::game_party->IsAnyActive()) {
		...
		if (Main_Data::game_party->GetBattlerCount() > 0) {
			if (Chaos::MultiplayerState::Instance().ShouldInterceptGameOver()) {
				return false;
			}
...
bool Game_Interpreter::CommandShowMessage(lcf::rpg::EventCommand const& com) { // code 10110
	...
	// Chaos Fork: AI Rewriter hook — collect whole event, rewrite once, pop per message
	if (Chaos::AIRewriter::IsEnabled()) {
		auto event_id = reinterpret_cast<uintptr_t>(list.data());
		bool has_face = !Main_Data::game_system->GetMessageFaceName().empty();

		// Determine dialogue-only line count (exclude choices)
		int dialogue_count = pm.HasChoices() ? pm.GetChoiceStartLine() : pm.NumLines();
		std::vector<std::string> dialogue_lines(pm.GetLines().begin(),
			pm.GetLines().begin() + dialogue_count);

		if (!Chaos::AIRewriter::HasCachedMessages(event_id)) {
			// First ShowMessage in this batch — scan ahead and collect ALL messages
			std::vector<std::vector<std::string>> all_messages;
			all_messages.push_back(dialogue_lines);

			// Peek ahead through the rest of the command list (read-only)
			for (int scan = index; scan < static_cast<int>(list.size()); ++scan) {
				if (static_cast<Cmd>(list[scan].code) == Cmd::ShowMessage) {
					std::vector<std::string> block;
					block.push_back(ToString(list[scan].string));
					int s2 = scan + 1;
					while (s2 < static_cast<int>(list.size()) &&
						   static_cast<Cmd>(list[s2].code) == Cmd::ShowMessage_2) {
						block.push_back(ToString(list[s2].string));
						++s2;
					}
					all_messages.push_back(std::move(block));
					scan = s2 - 1;
				}
			}

			Chaos::AIRewriter::CollectAndRewriteEvent(event_id, all_messages, has_face);
		}

		// Pop the rewritten lines matching THIS message's original text
		auto rewritten = Chaos::AIRewriter::PopRewrittenMessage(dialogue_lines);
		if (!rewritten.empty()) {
			auto& orig_lines = const_cast<std::vector<std::string>&>(pm.GetLines());

			if (pm.HasChoices()) {
				// Choices are appended after dialogue — preserve them.
				// Replace only the dialogue portion, keeping choice lines intact.
				int choice_start = pm.GetChoiceStartLine();
				std::vector<std::string> choice_lines(orig_lines.begin() + choice_start, orig_lines.end());

				orig_lines.clear();
				// Add rewritten dialogue lines (up to 4 minus choice room)
				int max_dialogue = 4 - static_cast<int>(choice_lines.size());
				for (size_t i = 0; i < rewritten.size() && static_cast<int>(i) < max_dialogue; ++i) {
					orig_lines.push_back(std::move(rewritten[i]));
				}
				// Re-append choices
				for (auto& cl : choice_lines) {
					orig_lines.push_back(std::move(cl));
				}
			} else {
				// No choices — replace all dialogue lines directly.
				// PopRewrittenMessage already limits to 4 lines and queues overflow.
				orig_lines.clear();
				for (auto& line : rewritten) {
