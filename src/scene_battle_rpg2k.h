...
protected:
	...
	void ApplyRemoteBattleAction(Game_Actor* actor, const Chaos::MultiplayerState::RemoteBattleAction& ra);
	...
	// Multiplayer: waiting for remote player to choose action
	bool waiting_for_remote_action = false;
	uint16_t waiting_remote_actor_id = 0;

	// Multiplayer: waiting for host to signal execution phase start
	bool waiting_for_turn_sync = false;

	// Multiplayer: waiting for escape vote result
	bool waiting_for_escape_vote = false;
};
