...
DirectoryTree::DirectoryListType* DirectoryTree::ListDirectory(std::string_view path) const {
	...
	if (dir_it != dir_cache.end()) {
		...
		if (file_it != fs_cache.end()) {
			return &file_it->second;
		}

		// Recover from a partially invalidated cache instead of asserting.
		DebugLog("ListDirectory Stale Cache Entry: {}", dir_key);
...
void DirectoryTree::ClearCache(std::string_view path) const {
	...
	fs_cache.erase(std::remove_if(fs_cache.begin(), fs_cache.end(), [&dir_key] (const auto& entry) {
		return entry.first == dir_key || StartsWith(entry.first, dir_key + "/");
	}), fs_cache.end());
	dir_cache.erase(std::remove_if(dir_cache.begin(), dir_cache.end(), [&dir_key] (const auto& entry) {
		return entry.first == dir_key || StartsWith(entry.first, dir_key + "/");
	}), dir_cache.end());
	dir_missing_cache.erase(std::remove_if(dir_missing_cache.begin(), dir_missing_cache.end(), [&dir_key] (const auto& dir) {
		return dir == dir_key || StartsWith(dir, dir_key + "/");
