...
void Game_Player::UpdateNextMovementAction() {
	...
	if (IsEncounterCalling()) {
		...
		if (Game_Map::PrepareEncounter(args)) {
			// Notify multiplayer about battle start
...
bool Game_Player::CheckEventTriggerHere(TriggerSet triggers, bool triggered_by_decision_key, bool face_player) {
	if (Chaos::MultiplayerState::Instance().IsSpectating()) return false;
	if (InAirship()) {
		return false;
	}
...
void Game_Player::ResetGraphic() {
	...
	if (mp.IsActive()) {
		if (mp.IsModeActive(Chaos::MultiplayerMode::Single) ||
			...
			// No actor for this player slot - enter spectator mode
			SetSpriteGraphic("", 0);
			SetTransparency(7);
			mp.EnterSpectatorMode();
			return;
		}
	}
	...
	if (terrain) {
		if (terrain->damage != 0) {
			...
			if (terrain->damage > 0 && terrain->easyrpg_damage_can_kill) {
				if (!Main_Data::game_party->IsAnyActive() && Main_Data::game_party->GetBattlerCount() > 0) {				if (Chaos::MultiplayerState::Instance().ShouldInterceptGameOver()) {
					return true;
