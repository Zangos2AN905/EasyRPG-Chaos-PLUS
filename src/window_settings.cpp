...
void Window_Settings::RefreshChaos() {
	...
	{
		Option opt;
		opt.text = "Discord Rich Presence";
		opt.help = "Show game status in your Discord profile";
		opt.value_text = Chaos::DiscordIntegration::IsEnabled() ? "[ON]" : "[OFF]";
		opt.mode = eOptionNone;
		opt.action = [this]() {
	...
	{
		Option opt;
		opt.text = "AI Rewriter";
		opt.help = "rewrites dialogue to be haha funny";
		opt.value_text = Chaos::AIRewriter::IsEnabled() ? "[ON]" : "[OFF]";
		opt.mode = eOptionNone;
		opt.action = [this]() {
			Chaos::AIRewriter::SetEnabled(!Chaos::AIRewriter::IsEnabled());
			Refresh();
		};
		GetFrame().options.push_back(std::move(opt));
	}

	// AI Characters: cycle OFF -> ADD -> REPLACE
	{
		Option opt;
		opt.text = "AI Characters";
		opt.help = "create your own characters and put them in your games, basically miitopia but for rpg maker";
		opt.value_text = std::string("[") + Chaos::AICharacters::GetModeString() + "]";
		opt.mode = eOptionNone;
		opt.action = [this]() {
			Chaos::AICharacters::CycleMode();
			Refresh();
		};
		GetFrame().options.push_back(std::move(opt));
	}
	...
	if (Chaos::AICharacters::GetMode() != Chaos::AICharacters::Mode::Off) {
		...
		int active_count = Chaos::AICharacters::GetActiveCount();
		std::string manage_val = std::to_string(active_count) + "/"
			+ std::to_string(Chaos::AICharacters::MAX_ACTIVE_CHARACTERS) + " active, "
			+ std::to_string(chars.size()) + " total";
		AddOption(MenuItem("  Manage Characters", "enable/disable/create/delete AI characters", manage_val), [](){
			Scene::Push(std::make_shared<Scene_AICharacters>());
		});
	}
	{
		Option opt;
		opt.text = "New Game+";
		opt.help = "Coming soon...";
		opt.value_text = "Coming soon";
		opt.mode = eOptionNone;
		GetFrame().options.push_back(std::move(opt));
	}
	*/
