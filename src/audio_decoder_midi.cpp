...
bool AudioDecoderMidi::SetPitch(int pitch) {
	...
	this->pitch = pitch;
	// Native MIDI uses pitch as playback tempo. Apply a note transpose too
	// so low-pitch effects actually lower the octave of MIDI notes.
	midi_note_shift = pitch < 80 ? -12 : 0;
	return true;
}
...
void AudioDecoderMidi::midi_message(int, uint_least32_t message) {
	...
	if ((event_type == 0x8 || event_type == 0x9) && midi_note_shift != 0) {
		const int shifted_note = std::clamp(
			static_cast<int>(value1) + midi_note_shift, 0, 127);
		message = midimsg_make(event_type, channel,
			static_cast<uint8_t>(shifted_note), value2);
		value1 = static_cast<uint8_t>(shifted_note);
	}
