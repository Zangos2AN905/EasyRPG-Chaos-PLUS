...
void Player::Run() {
	...
	Chaos::DiscordIntegration::Initialize();
...
void Player::MainLoop() {
	...
	Chaos::DiscordIntegration::Update();
	Chaos::MultiplayerState::Instance().Update();

	// If host disconnected, kick client back to title/game browser
	if (Chaos::MultiplayerState::Instance().IsHostDisconnected()) {
...
void Player::Exit() {
	Chaos::DiscordIntegration::Shutdown();
	Chaos::MultiplayerState::Instance().StopMultiplayer();
	Chaos::NetManager::Instance().Shutdown();
