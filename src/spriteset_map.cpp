...
void Spriteset_Map::Refresh() {
	...
	if (mp.IsActive()) {
		// OnMapLoaded creates sprites for every remote player on this map.
		// Do not also loop here — that would add duplicate sprites.
		mp.OnMapLoaded(this);
	}
...
void Spriteset_Map::AddCharacterSprite(Game_Character* character) {
	CreateSprite(character, need_x_clone, need_y_clone);
}

void Spriteset_Map::RemoveCharacterSprites(Game_Character* character) {
	character_sprites.erase(
		std::remove_if(character_sprites.begin(), character_sprites.end(),
