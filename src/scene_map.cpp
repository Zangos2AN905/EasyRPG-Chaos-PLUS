...
Scene_Map::~Scene_Map() {
	...
	Chaos::MultiplayerState::Instance().OnMapUnloaded();
}
...
void Scene_Map::vUpdate() {
	...
	// Apply host event-message input after Input::Update and before the map
	// update, so the normal message/interpreter code consumes it this frame.
	Chaos::MultiplayerState::Instance().ApplyPendingEventInput();
...
void Scene_Map::UpdateSceneCalling() {
	...
	// Handle forced battle from Chaotix mode
	if (call == nullptr) {
		auto& mp = Chaos::MultiplayerState::Instance();
		if (mp.IsActive() && mp.HasForcedBattle()) {
