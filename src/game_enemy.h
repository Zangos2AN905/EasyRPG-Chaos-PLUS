...
inline int Game_Enemy::GetBaseMaxHp() const {
	...
	if (Chaos::SplitMode::Instance().EnemyHealthMultiplier() > 1) {
		const auto multiplier = Chaos::SplitMode::Instance().EnemyHealthMultiplier();
		max_hp = max_hp > MaxHpValue() / multiplier
			? MaxHpValue() : max_hp * multiplier;
	}
	return max_hp;
}
