...
Scene_Battle_Rpg2k::SceneActionReturn Scene_Battle_Rpg2k::ProcessSceneActionFightAutoEscape() {
	...
	if (scene_action_substate == eBegin) {
		ResetWindows(true);
		battle_message_window->SetVisible(true);
		waiting_for_escape_vote = false;
		Chaos::MultiplayerState::Instance().ResetEscapeVote();
	...
	if (scene_action_substate == eWaitForInput) {
		...
		// Multiplayer escape vote waiting state
		if (waiting_for_escape_vote) {
			if (mp.HasEscapeVoteResult()) {
				waiting_for_escape_vote = false;
				if (mp.IsEscapeApproved()) {
					mp.ResetEscapeVote();
					SetState(State_Escape);
				} else {
					mp.ResetEscapeVote();
					// Other player chose to fight - force into fight mode
					battle_message_window->Clear();
					battle_message_window->Push("Escape denied!");
					SetState(State_SelectActor);
				}
				return SceneActionReturn::eContinueThisFrame;
			}
			// Still waiting - show message
			options_window->SetActive(false);
			options_window->SetVisible(false);
			battle_message_window->Clear();
			battle_message_window->SetVisible(true);
			battle_message_window->Push("Voted to escape. Waiting...");
			return SceneActionReturn::eWaitTillNextFrame;
		}
...
Scene_Battle_Rpg2k::SceneActionReturn Scene_Battle_Rpg2k::ProcessSceneActionActor() {
	// If waiting for host to signal execution phase start
	if (waiting_for_turn_sync) {
		auto& mp = Chaos::MultiplayerState::Instance();
		if (mp.HasTurnSync()) {
			waiting_for_turn_sync = false;
			int32_t seed = mp.ConsumeTurnSync();
			Rand::SeedRandomNumberGenerator(seed);
		...
		battle_message_window->SetVisible(true);
		battle_message_window->Clear();
		battle_message_window->Push("Syncing battle...");
		return SceneActionReturn::eWaitTillNextFrame;
	}
	// If waiting for a remote player's action, poll until it arrives
	if (waiting_for_remote_action) {
		auto& mp = Chaos::MultiplayerState::Instance();
		if (mp.HasRemoteAction(waiting_remote_actor_id)) {
			// Action arrived - apply it and continue
			waiting_for_remote_action = false;
			std::vector<Game_Actor*> allies = Main_Data::game_party->GetActors();
			// Find the actor we were waiting for (actor_index was already incremented past them)
			Game_Actor* waiting_actor = nullptr;
			for (auto* a : allies) {
				if (static_cast<uint16_t>(a->GetId()) == waiting_remote_actor_id) {
					waiting_actor = a;
					break;
				}
			}
			if (waiting_actor) {
				auto ra = mp.ConsumeRemoteAction(waiting_remote_actor_id);
				ApplyRemoteBattleAction(waiting_actor, ra);
				assert(waiting_actor->GetBattleAlgorithm() != nullptr);
				battle_actions.push_back(waiting_actor);
			}
			battle_message_window->Clear();
			// Continue processing remaining actors
			SelectNextActor(false);
			return SceneActionReturn::eContinueThisFrame;
		}
		// Still waiting - show message and wait
		ResetWindows(true);
		battle_message_window->SetVisible(true);
		battle_message_window->Clear();
		battle_message_window->Push("Waiting for other player...");
		return SceneActionReturn::eWaitTillNextFrame;
	}
	SelectNextActor(false);
	return SceneActionReturn::eContinueThisFrame;
...
void Scene_Battle_Rpg2k::SelectNextActor(bool auto_battle) {
	...
	if ((size_t)actor_index == allies.size()) {
		...
		if (mp.IsActive() && mp.IsInMultiplayerBattle()) {
			// Derive a per-turn seed for deterministic execution
			int32_t turn_count = Main_Data::game_party->GetTurns();
			int32_t turn_seed = mp.GetBattleSeed() + turn_count + 1;
			if (net.IsHost()) {
				// Host: broadcast turn sync and proceed immediately
				mp.BroadcastTurnSync(turn_seed);
				Rand::SeedRandomNumberGenerator(turn_seed);
				SetState(State_Battle);
				NextTurn();
				CreateEnemyActions();
				CreateExecutionOrder();
				...
				// Client: wait for host's turn sync signal
				waiting_for_turn_sync = true;
				return;
			}
		}
	...
	// In team/chaotix mode, auto-battle actors not controlled by this player
	auto& mp = Chaos::MultiplayerState::Instance();
	if (mp.IsActive() && mp.IsRemoteActor(active_actor)) {
		// Check if we have a remote action for this actor
		if (mp.HasRemoteAction(static_cast<uint16_t>(active_actor->GetId()))) {
			auto ra = mp.ConsumeRemoteAction(static_cast<uint16_t>(active_actor->GetId()));
			ApplyRemoteBattleAction(active_actor, ra);
			assert(active_actor->GetBattleAlgorithm() != nullptr);
			battle_actions.push_back(active_actor);
			SelectNextActor(auto_battle);
			return;
		} else {
			// Wait for the remote player to choose their action
			waiting_for_remote_action = true;
			waiting_remote_actor_id = static_cast<uint16_t>(active_actor->GetId());
			return;
		}
	}
	...
	if (auto_battle || active_actor->GetAutoBattle()) {
		...
		// In multiplayer, send auto-battled local actor's action to remote players
		if (mp.IsActive() && !mp.IsRemoteActor(active_actor)) {
			auto* algo = active_actor->GetBattleAlgorithm().get();
			if (algo) {
				uint8_t action_type = 0;
				switch (algo->GetType()) {
					case Game_BattleAlgorithm::Type::Normal:  action_type = 1; break;
					case Game_BattleAlgorithm::Type::Skill:   action_type = 2; break;
					case Game_BattleAlgorithm::Type::Item:    action_type = 3; break;
					case Game_BattleAlgorithm::Type::Defend:  action_type = 4; break;
					default: action_type = 0; break;
				}
				mp.SendBattleAction(
					static_cast<uint16_t>(active_actor->GetId()),
					action_type, 0, 0);
			}
		}
...
void Scene_Battle_Rpg2k::ActionSelectedCallback(Game_Battler* for_battler) {
	...
	// In team/chaotix mode, send action over network for our actor
	if (for_battler->GetType() == Game_Battler::Type_Ally) {
		auto& mp = Chaos::MultiplayerState::Instance();
		if (mp.IsActive() && !mp.IsRemoteActor(static_cast<Game_Actor*>(for_battler))) {
			auto* algo = for_battler->GetBattleAlgorithm().get();
			if (algo) {
				uint8_t action_type = 0;
				int32_t target_id = 0;
				...
				switch (algo->GetType()) {
					case Game_BattleAlgorithm::Type::Normal:
						action_type = 1;
						break;
					case Game_BattleAlgorithm::Type::Skill:
						action_type = 2;
						break;
					case Game_BattleAlgorithm::Type::Item:
						action_type = 3;
						break;
					case Game_BattleAlgorithm::Type::Defend:
						action_type = 4;
						break;
					default:
						action_type = 0;
				...
				mp.SendBattleAction(
					static_cast<uint16_t>(static_cast<Game_Actor*>(for_battler)->GetId()),
					action_type, target_id, skill_or_item_id);
			}
		}
		SetState(State_SelectActor);
	}
...
void Scene_Battle_Rpg2k::ApplyRemoteBattleAction(Game_Actor* actor, const Chaos::MultiplayerState::RemoteBattleAction& ra) {
	switch (ra.action_type) {
		case 1: // Normal attack
			actor->SetBattleAlgorithm(std::make_shared<Game_BattleAlgorithm::Normal>(
				actor, Main_Data::game_enemyparty.get()));
			break;
		case 2: { // Skill
			const auto* skill = lcf::ReaderUtil::GetElement(lcf::Data::skills, ra.skill_or_item_id);
			if (skill) {
				actor->SetBattleAlgorithm(std::make_shared<Game_BattleAlgorithm::Skill>(
					actor, Main_Data::game_enemyparty.get(), *skill));
			} else {
				actor->SetBattleAlgorithm(std::make_shared<Game_BattleAlgorithm::Normal>(
					actor, Main_Data::game_enemyparty.get()));
			}
		...
		case 3: { // Item
			const auto* item = lcf::ReaderUtil::GetElement(lcf::Data::items, ra.skill_or_item_id);
			if (item) {
				actor->SetBattleAlgorithm(std::make_shared<Game_BattleAlgorithm::Item>(
					actor, Main_Data::game_party.get(), *item));
			} else {
				actor->SetBattleAlgorithm(std::make_shared<Game_BattleAlgorithm::Normal>(
					actor, Main_Data::game_enemyparty.get()));
			}
		...
		case 4: // Defend
			actor->SetBattleAlgorithm(std::make_shared<Game_BattleAlgorithm::Defend>(actor));
			break;
		default:
			actor->SetBattleAlgorithm(std::make_shared<Game_BattleAlgorithm::Normal>(
				actor, Main_Data::game_enemyparty.get()));
